...
class Image(object):
	...
	def clear(self, color = Color(255, 255, 255, 255)):
		#clear image to white by default
		row = bytearray(1) + bytearray([color.r(), color.g(), color.b(), color.a()] * self.width)
		self.buffer = row * self.height

	def displayBuffer(self):
		np_buffer_png = np.frombuffer(self.buffer, dtype=np.uint8)
		#the buffer has an extra null bit at the start of each row that needs to be removed, so i reshape, slice and then reshape again
		np_buffer_png = np_buffer_png.reshape((self.height, self.width * 4 + 1))
		np_buffer_RGB = np_buffer_png[:, 1:]
		np_buffer_RGB = np_buffer_RGB.reshape((self.height, self.width, 4))
		#OpenCV uses BGR instead of RGB
		np_buffer_BGR = cv2.cvtColor(np_buffer_RGB, cv2.COLOR_RGBA2BGRA)

		cv2.imshow('VR Headset Scene Render', np_buffer_BGR)
